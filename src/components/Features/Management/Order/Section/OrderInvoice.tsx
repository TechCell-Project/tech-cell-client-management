...
const OrderInvoice = () => {
  ...
  const renderLine = (label: string, content: string, isRed?: boolean) => {
    return (
      <Stack flexDirection='row' justifyContent='space-between' alignItems='center'>
        <Typography fontSize='15px' fontStyle='italic'>{label}</Typography>
        <Typography fontSize='15px' color={isRed ? 'secondary' : ''}
          fontWeight={600}>{isRed ? `- ${content}` : content}</Typography>
      </Stack>
    );
  };
  ...
  return (
    <>
            ...
            <>
              <Stack flexDirection='row' alignItems='flex-start' key={i} gap='20px' width='100%' mt={3} mb='20px'>
                <Avatar
                  src={String(image)}
                  alt='User Avatar'
                  sx={{ height: '80px', width: '80px', borderRadius: '5px' }} variant='square'
                />
                <Stack flexDirection='column' width='100%'>
                  <Typography fontSize='16px' fontWeight={600}>{item.name}</Typography>
                  <Typography
                    fontSize='15px'
                    fontWeight={500}
                  ...
                  <Typography fontSize='15px' fontWeight={600}
                    textAlign='right'>{formatWithCommas(Number(item.price.base))}</Typography>
                </Stack>
              </Stack>
      ...
      <Stack flexDirection='row' justifyContent='space-between' alignItems='center' width='100%' mt={3}>
        <Typography fontSize='17px' fontWeight={600} fontStyle='italic'>Tổng tiền</Typography>
        <Typography fontSize='17px'
          fontWeight={700}>{formatWithCommas(Number(order?.checkoutOrder.totalPrice))}</Typography>
      </Stack>
    </>
  );
};
